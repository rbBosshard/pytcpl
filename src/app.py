        ...
        if best:
            ...
            for potency in potencies:
                if potency in series:
                    pot = np.log10(series[potency])
                    if pot is not None and not np.isnan(pot):
                        pot_y = get_model(fit_model)(np.array([series[potency]]), **params)[0]
                        fig.add_trace(go.Scatter(
                            x=[pot, pot],
                            y=[-20, pot_y],  # Coordinates to cover the entire vertical range of the plot
                            mode='lines',
...
def update():
    ...
    refresh_load = fresh_load or st.session_state.trigger in ["spid", "hitcall_slider"]
    if refresh_load:
    ...
    placeholder_hitcall_slider.write(f"{length} series in filter")

    if st.session_state.trigger in ["sort_by", "asc", "hitcall_slider"]:
        sort()
    
    if st.session_state.trigger == "next":
        st.session_state.id += 1
    if st.session_state.trigger == "prev":
        st.session_state.id -= 1
    
    if st.session_state.trigger == "spid" and st.session_state.spid != "":
    ...
    with st.expander("Sample info", expanded=True):
        st.dataframe(df,
            column_config= {
                ...
                "DSSTOXSID": st.column_config.LinkColumn("DSSTOXSID", help="Distributed Structure-Searchable Toxicity Substance ID. Link to dashboard :bar_chart:"),
                "CASRN": st.column_config.TextColumn(label="CASN", help="CAS Registry Number :information_source:"),
            },
            hide_index=True,
