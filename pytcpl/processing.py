...
def preprocess(df, test):
    ...
    df['concentration_unlogged'], df['response'] = zip(*df.apply(filter_concentrations_and_responses, axis=1))
    # Truncate to head according to test
    df = df.head(test) if test else df  # work only with subset if test > 1
