...
def write_lvl_4(dat):
    ...
    #unnest fit2 params
    unnested_param = pd.concat([pd.DataFrame(tcplFit2_unnest(x)) for x in param['fitparams']], keys=param['m4id'], names=['m4id'])
    unnested_param.reset_index(inplace=True)
    unnested_param = unnested_param.set_index("m4id")
    param = param.set_index("m4id")
    dat1 = param.join(unnested_param, how="left")
    dat_param = dat1[["aeid", "model", "model_param", "model_val"]]
    dat_param.reset_index(inplace=True)
...
def tcplFit2_unnest(output):
    ...
    test = pd.DataFrame.from_records(rows, columns=["model", "model_param", "model_val"])
